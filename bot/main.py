...
def get_keyboard(state: str, user=None, db=None) -> str:
    ...
    if is_legend(state):
        from bot.keyboard import legend_buttons_keyboard, legend_waiting_keyboard
        if is_legend_waiting(state):
            return legend_waiting_keyboard()
...
def main():
    ...
    for event in longpoll.listen():
        ...
        try:
            ...
            if cmd == "open_payment":
                handle_open_payment(user, db, send_message)
                continue
            ...
            elif cmd == "activate_all":
                handle_activate_all(user, db, send_message)
            elif cmd == "grow":

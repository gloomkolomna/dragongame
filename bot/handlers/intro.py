...
from bot.keyboard import intro_keyboard, intro_last_keyboard, empty_keyboard, buy_eggs_row, bestiary_link_row, row, _keyboard

_INTRO_IMAGES = os.path.join(os.path.dirname(__file__), "..", "..", "images", "intro")


def _post_intro_keyboard(db, vk_id: int):
    buttons = []
    try:
        from services.payment_service import is_donor
        if is_donor(vk_id, db):
            buttons.append(row(("\U0001f511 Мои PIN-коды", "my_pins")))
    except Exception:
        pass
    buttons.append(buy_eggs_row())
...
def start_intro(user, db, send_message, upload_image=None):
    ...
    if not chapter:
        ...
        db.commit()
        send_message("Введи PIN-код с яйца, чтобы начать выращивание.", keyboard=_post_intro_keyboard(db, user.vk_id))
        return
...
def handle_intro_next(user, db, send_message, upload_image=None):
    ...
    if not chapter:
        ...
        send_message(
            ...
            keyboard=_post_intro_keyboard(db, user.vk_id),
        )
        return

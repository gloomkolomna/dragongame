...
сообщение с количеством вышитых стежков (например, «вышито 1500»),
...
- **Сначала пояснение, потом код.** Перед любым изменением кода — объяснить, что именно будет сделано и почему. Дождаться утверждения пользователем. Только после утверждения писать код. Не начинать с правок без предварительного согласования.
...
/ Коллекция драконов/
  ...
  bot/
    ...
    handlers/
      ...
      grow.py         # Обработка шагов (стежки + норма/штраф)
...
## Выращивание — механика стежков

1. Поле `crosses_norm` на `DragonStep` — норма стежков (целое, ≥ 1, по умолчанию 1000)
...
- `is_waiting_text(state)` — проверка, ждёт ли бот сообщение с стежками
...
- `await_rules` — меню раздела «Правила игры»; сохраняет `_rules_prev_state` в `state_data`, при закрытии восстанавливает прежнее состояние
- Магазин работает без отдельного FSM-состояния (по payload-командам `shop`/`buy`)

## Клавиатуры бота

- `idle_keyboard()` — Магазин / Список Бестиария / Правила / Помощь / Бестиарий (без «Добавить яйцо»)
- `growing_keyboard()` — Магазин / Список Бестиария / Правила / Помощь / Бестиарий (без «Перейти»)
- `start_growing_keyboard()` — Перейти к выращиванию / Список Бестиария / Правила / Помощь / Бестиарий (только при активации PIN)
- `waiting_keyboard()` — Назад / Список Бестиария / Правила / Помощь / Бестиарий (режим ожидания стежков)
- `step_buttons_keyboard()` — Норма / Штраф(x2) / Список Бестиария / Правила / Помощь / Бестиарий
- `await_pin_keyboard()` — Список Бестиария / Правила / Помощь / Бестиарий (при вводе PIN)
- `epic_name_keyboard()` — при вводе имени эпического (Список Бестиария / Помощь / Бестиарий; служебные слова garden/help/rules/start обрабатываются как команды, а не как имя)
- `epic_restart_keyboard()` — после финала эпического (Такого же заново / Случайного / Список Бестиария / Помощь / Бестиарий)
- `await_garden_keyboard(with_cancel=True)` — раздел «Список Бестиария»: Добавить яйцо + (Не менять) + Бестиарий. Единственная клавиатура с кнопкой «🥚 Добавить яйцо дракона» (помимо `_completed_keyboard`)
- `shop_keyboard(items, page, total_pages)` — товары стадии + пагинация + Мой инвентарь + К эпическому дракону + Список Бестиария (магазин)
- `inventory_keyboard()` — Магазин / К эпическому дракону / Список Бестиария / Бестиарий (инвентарь)
- `legend_buttons_keyboard()` — Норма / Штраф(x2) / Список Бестиария / Бестиарий (легенда)
- `epic_egg_buttons_keyboard()` — Норма / Штраф(x2) / Список Бестиария / Бестиарий (яйцо эпического)
- `epic_care_keyboard()` — Норма / Штраф(x2) / Магазин / Список Бестиария / Бестиарий (уход)
- `care_shop_keyboard()` — Магазин / К дракону / Список Бестиария / Правила / Помощь / Бестиарий (нужны товары для действия; «◀ К дракону» возвращает к текущему действию ухода)
- `sub_step_keyboard()` — Норма / Штраф(x2) / Список Бестиария / Бестиарий (шаг составного действия)
...
- Кнопка «📖 Список Бестиария» (garden) присутствует на всех клавиатурах, КРОМЕ `await_garden_keyboard` (внутри самого раздела «Список Бестиария» garden-кнопка не нужна)
- Кнопка «🥚 Добавить яйцо дракона» (pin) присутствует ТОЛЬКО в `await_garden_keyboard` (раздел «Список Бестиария») и `_completed_keyboard` (выращенные драконы). В остальных клавиатурах отсутствует
...
- `help` / `помощь` — справка
- `rules` / `правила` / `/rules` — меню разделов «Правила игры» (8 разделов: выращивание, таймауты, редкости, эпические, легенды, сокровища, магазин, честная игра); сохраняет текущее состояние, при закрытии восстанавливает
- `rules_section` (payload `{"cmd":"rules_section","section":"..."}`) — показать конкретный раздел правил (кнопка из меню)
- `rules_close` — закрыть правила (восстановить прежнее состояние)
...
- `balance` / `копилка` / `баланс` — показать копилку стежков
- `shop` / `магазин` / `лавка` — магазин стадии; `buy` (payload `{"cmd":"buy","item_id":N}`) — покупка (после успешной покупки возврат к действию эпического); `inventory` / `инвентарь` — список купленного с количеством

...
> **Схема изображений:** у дракона два пути — `egg_path` (яйцо, показывается
> в процессе выращивания) и `dragon_path` (взрослый дракон, показывается после
...
> **Важно:** `python-multipart` обязателен — без него роуты с `Form(...)`/`File(...)`
> (создание дракона, загрузка изображений) падают при импорте с RuntimeError.
...
Полноценный VK longpoll бот (не заглушка): `bot/main.py` — longpoll-слушатель с клавиатурой, FSM-состояниями и обработчиками (`handlers/commands.py`, `pin.py`, `grow.py`). Для локального теста:
...
Бот использует тот же venv, что и API, и импортирует `api.db`/`api.models`/`api.config` (каталог `api/` добавлен в `sys.path` в `bot/main.py`).
...
- Привязка PIN к игроку реализована: при вводе PIN боту создаётся запись `user_dragons`, дракон появляется в коллекции игрока (яйцо с прогрессом → взрослый дракон после выполнения всех шагов). Таблица PIN-кодов в админке: Дракон | PIN-код | Редкость | Тип яйца | Шагов | Активен

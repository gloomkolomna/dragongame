...
function EpicStageForm() {
    ...
    if (isEdit) {
      client.get('/admin/epic/stages').then((r) => {
        ...
        if (s) {
          ...
          setImageStart(s.image_start || ''); setImageEnd(s.image_end || '');
        }
  ...
  const save = async () => {
    ...
    const payload: any = { stage_number: stageNumber, name, description, image_start: imageStart, image_end: imageEnd };
    try {

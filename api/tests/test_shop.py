...
def test_stage_key_egg(db):
    _epic_egg_user(db)
    assert shop_service.get_current_stage_key(db, 100) == "epic:egg"


def test_stage_key_care_stage(db):
    _epic_egg_user(db)
    stage = EpicStage(stage_number=2, name="S2")
    db.add(stage)
    db.flush()
    ud = db.query(UserDragon).filter(UserDragon.user_id == 100).first()
    db.add(EpicCareState(user_dragon_id=ud.id, stage_id=stage.id))
    db.commit()
    assert shop_service.get_current_stage_key(db, 100) == "epic:2"
...
def test_get_stage_items_filters_active(db):
    ...
    db.add(StageShopItem(stage_key="epic:egg", item_id=a.id))
    db.add(StageShopItem(stage_key="epic:egg", item_id=b.id))
    db.commit()
    items = shop_service.get_stage_items(db, "epic:egg")
...
def test_purchase_insufficient(db):
    _epic_egg_user(db, vk_id=101, balance=50)
    it = _item(db, cost=300)
    db.add(StageShopItem(stage_key="epic:egg", item_id=it.id))
    db.commit()
...
def test_api_shop_and_inventory(client, db):
    ...
    assert shop["stage_key"] == "epic:egg"

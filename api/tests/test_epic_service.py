...
def test_advance_cycle_to_finale(db):
    ...
    st, _ = _stage(db, number=1, cycles=2, actions=2)
...
def test_advance_stage_up(db):
    ...
    _stage(db, number=1, cycles=1, actions=1)
    st2, _ = _stage(db, number=2, cycles=1, actions=1)
...
def test_missing_items_and_character(db):
    ...
    st, _ = _stage(db, number=1, cycles=1, actions=0)
